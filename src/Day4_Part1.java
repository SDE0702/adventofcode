...
""";
	...
	public static void main(String[] args) throws Exception 
	{
		...
		int i = 0;
		ArrayList<HashMap<String, String>> personalInformations = new ArrayList<>();
		
		HashMap<String, String> pi = null;
		while (true)
		{
		...
		int cc = 0;
		boolean Part_1 = false;
			for(HashMap<String,String> pix : personalInformations)
			{
				if (Part_1) {
					if (pix.get("byr") != null &&
						pix.get("iyr") != null &&
...
*/					
						...
						
					{
						...
						String byrs = pix.get("byr");
						int byr = Integer.valueOf(byrs);
						...
						int iyr = Integer.valueOf(pix.get("iyr"));
						if (iyr >= 2010 && iyr <= 2020)
						{
							validIssue = true;
						}
	
						boolean validExpiration = false;
						int eyr = Integer.valueOf(pix.get("eyr"));
						...
						if (pix.get("hgt").contains("cm"))
						{
							int hgt = Integer.valueOf(pix.get("hgt").replace("cm",""));
							if (hgt >= 150 && hgt <= 193)
							{
								validHigh = true;
							}
						} else if (pix.get("hgt").contains("in"))
						{
							int hgt = Integer.valueOf(pix.get("hgt").replace("in",""));
							if (hgt >= 59 && hgt <= 76)
							{
								validHigh = true;
							}
						}
						boolean validHairColor = false;
						if (pix.get("hcl").length() == 7 &&  pix.get("hcl").matches("#[0-9a-f].*"))
						{
							validHairColor = true;
						} else {
						...
						boolean validEyeColor = false;
						if (pix.get("ecl").matches("amb|blu|brn|gry|grn|hzl|oth"))
						{
							validEyeColor = true;
						} else {
							validEyeColor = false;
						...
						if (pix.get("pid").length() == 9 && pix.get("pid").matches("[0-9].*"))
						{
							validPid = true;
						}
						
						if (validBirthday && validExpiration && validEyeColor && validHairColor && validHigh && validIssue && validPid)
						{
							cc++;
						}
					}					
