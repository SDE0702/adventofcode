...
private static String shiny_gold = "shiny gold";
	...
	public static void main(String[] args) throws Exception 
	{
		...
		HashMap<String,HashMap<String,Integer>> liste = new HashMap<>();
		...
		while (true)
		{
			...
			String[] tok1 = line.split(" contain ");
			...
			{
				t = t.trim();
				if (!t.startsWith("no other"))
				{
					int pos = t.indexOf(' ');
					String x = t.substring(0,pos);
					Integer anz = Integer.parseInt(x);
					x = t.substring(pos).replace(".","").replace("bags","").replace("bag","").trim();
					hm.put(x, anz);
				}
			}
			liste.put(tok1[0].replace("bags","").replace("bag","").trim(), hm);
		}
...
//		System.out.println(liste);
		...
		{
			if (!k.equals(shiny_gold))
			{	
				found = false;   
				runTrougthList(k,liste);
				if (found)
				{
					cc++;
				}
...
//		System.out.println("different bag count with " + shiny_gold + ":" + cc);
		
		// Part2
		cc = 0;
		traverseGoldBags(shiny_gold,1,liste);
		
		System.out.println(cc);
		
		
	}
	private static void traverseGoldBags(String color,int multiplicator,HashMap<String, HashMap<String, Integer>> liste) 
	{
		HashMap<String, Integer> entry = liste.get(color);
		System.out.println(" ".repeat(indent) + color + ",mult=" + multiplicator + ":" + entry );
		indent += 4;
		for(String c : entry.keySet())
		{
			Integer m = entry.get(c);
			cc += m * multiplicator;
			traverseGoldBags(c, m * multiplicator, liste);
		}
		indent -= 4;
	}
...
private static boolean found = false;
	private static void runTrougthList(String color, HashMap<String, HashMap<String, Integer>> liste) 
	{
		HashMap<String, Integer> entry = liste.get(color);
//		System.out.println(" ".repeat(indent) +  color + "  " + entry.keySet());
//		indent += 4;
		if (entry != null && !found)
		{
			for(String c : entry.keySet())
			{
				if (c.equals("shiny gold"))
				{
					found = true;
					break;
				}
				runTrougthList(c, liste);
			}
		}
//		indent -= 4;
	}
