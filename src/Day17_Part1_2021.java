import java.util.ArrayList;

public class Day17_Part1_2021 
{
	private static String real = "";
	private static String test1 = "EE00D40C823060";

	private static String input = test1;
	
	public static void main(String[] args) throws Exception 
	{
		StringBuilder sb = new StringBuilder();
		for(int i = 0; i < input.length();i += 2)
		{
			int n = Integer.parseInt(test1.substring(i,i+2),16);
			String b = Integer.toString(n,2);
			b = "0".repeat(4 - b.length()) + b;
			
			sb.append(b);
		}
		String r = sb.toString();
		
		System.out.println(r);

		ArrayList<Integer> pos = new ArrayList<>();
		pos.add(0);
		
		int version = getNextValue(r,pos,3);
		int id = getNextValue(r,pos,3);
		
		System.out.println("Version:" + version + " - ID:" + id);
		
		if (id == 4) // literal number
		{
			int cf = 0;
			int total = 0;
			do {
				cf = getNextValue(r, pos, 1);
				int v = getNextValue(r, pos, 4);
				total = total * 16 + v;
			} while (cf == 1);
			System.out.println("literalvalue=" + total);
		} else {  // Operator
			int lengthType = getNextValue(r, pos, 1);
			if (lengthType == 0)  // next 15 Bits --> length of sub packet
			{
				int lengthOfSubPackets = getNextValue(r, pos, 15);
				System.out.println("lengthOfSubPackets="+ lengthOfSubPackets);
			} else { // next 11 Bits --> total length of sub packet
				int numberOfSubPackets = getNextValue(r, pos, 11);
				System.out.println("numberOfSubPackets="+ numberOfSubPackets);
			}
		}
	}

	private static int getNextValue(String r, ArrayList<Integer> pos,int len) 
	{
		int p = pos.remove(0);
		
		String sub = r.substring(p,p + len);
		p += len;
		pos.add(p);
		int n = Integer.parseInt(sub,2); 
		return n;
	}
}
