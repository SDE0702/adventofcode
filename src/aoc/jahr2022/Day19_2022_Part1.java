...
//	private static String input = real;
	...
	public static final int MAXMINUTE = 24;
	...
	public static void main(String[] args) throws Exception 
	{
		...
		while (true)
		{
			...
			sumQualityLevel += getQualityLevel(line);
		}
		System.out.println(sumQualityLevel);
	}

/*
 * ore robot    clay robot obsidian robot   geode robot
1		6		12			18	   21		27	    30
2:		2 ore	3 ore		3 ore  8 clay	3 ore	12 obsidian.
 */
	
	
	private static int getQualityLevel(String line) 
	{
		int bluePrintNr;
		...
		int geodeRobotsCostOre;
		int geodeRobotsCostobsidian;

		String[] tok = line.replace("  "," ").split(" ");
		bluePrintNr = Integer.parseInt(tok[1].replace(":","").trim());
		oreRobotsCostOres = Integer.parseInt(tok[6].trim());
		
		clayRobotsCostOres = Integer.parseInt(tok[12].trim());
		
		obsidianRobotsCostOres = Integer.parseInt(tok[18].trim());
		obsidianRobotsCostClays = Integer.parseInt(tok[21].trim());
		
		geodeRobotsCostOre = Integer.parseInt(tok[27].trim());
		geodeRobotsCostobsidian = Integer.parseInt(tok[30].trim());
		
		
		ProductionStatus ps = new ProductionStatus(bluePrintNr,oreRobotsCostOres,clayRobotsCostOres,obsidianRobotsCostOres,obsidianRobotsCostClays,geodeRobotsCostOre,geodeRobotsCostobsidian);
		ps.printBluprintvalues();
		
		for(int minute = 1;minute <= MAXMINUTE;minute++)
		{
			System.out.println("Step minute:" + minute);
			ps.doProductionStep();
