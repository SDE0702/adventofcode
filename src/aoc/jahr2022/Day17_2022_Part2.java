...
//	private static final int MAXROCKS = (int)2022;
	private static final int MAXROCKS = 2000;
	private static final int MAXCHAMPERX = 7;
	private static HashSet<XYPoint> champer = new HashSet<>(); 
	private static ArrayList<Integer> champerBelegung = new ArrayList<Integer>();
...
/*		
		...
		while (ysearchcc < maxSearch && ysearchpos < maxy)
		{
			if (champerBelegung.get(ysearchcc) == champerBelegung.get(ysearchpos))
			{
				ysearchcc++;
			} else {
				ysearchcc = 0;
		...
		if (ysearchcc >= maxSearch)
		{
			System.out.println("Pos:" + ysearchpos);
		} else {
...
*/		
	...
	private static boolean checkFree(Rock r, int currentrockPosx, int currentrockPosy,int xoffset,int yoffset) 
	{
		...
		return (champer.containsAll(checkList));
	}
