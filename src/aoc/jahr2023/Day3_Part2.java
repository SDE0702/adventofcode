...
import aoc.jahr2023.Day3.NumbersInLineInfo;
...
//	public static String input = testInput;
	...
	public static void main(String[] args) throws Exception 
	{
		...
		while (true)
		{
			...
			// Debug
//			ArrayList<NumbersInLineInfo> info = getAllNumberInline(line);
//			System.out.println(line + ":" + info);
	...
	private static void scanLines(ArrayList<String> lines) 
	{
		long sumOfGears = 0;
		for(int i = 0;i < lines.size();i++)
		{
			// Search * and isolate numbers later
			...
			for(int j = 0;j < l.length();j++)
			{
				if (l.charAt(j) == '*')
				{
					...
					int gearMultiplikator = checkNumbersNearBy(lines,i,j);
					sumOfGears += gearMultiplikator;	
				}
			}
		}
...
*/
	...
	private static int checkNumbersNearBy(ArrayList<String> lines, int lineNumber, int pos) 
	{
		// Check Above
		int mult = 1;
		int countOf = 0;
		...
		if (lineNumber > 0)
		{
			String l = lines.get(lineNumber - 1);
			ArrayList<NumbersInLineInfo> info = getAllNumberInline(l);
			
			for(NumbersInLineInfo i : info)
			{
				int checkStartPos = Math.max(i.start-1,0);
				int checkEndPos = Math.min(i.end+1,l.length()-1);
				
				if (pos >= checkStartPos && pos <= checkEndPos)
				{
					mult *= i.number;
					countOf++;
					if (countOf > 2) 
					{
						System.out.println("Count above is too high in check linenumber " + lineNumber + "  pos = " + pos);
						System.exit(1);
					}
				}
			}
		...
		if (lineNumber < lines.size() -1)
		{
			...
			for(NumbersInLineInfo i : info)
			{
				...
				if (pos >= checkStartPos && pos <= checkEndPos)
				{
					...
					if (countOf > 2) 
					{
						System.out.println("Count below is too high in check linenumber " + lineNumber + "  pos = " + pos);
						System.exit(1);
					}
				}
			}
	...
	private static ArrayList<NumbersInLineInfo> getAllNumberInline(String l) 
	{
		ArrayList<NumbersInLineInfo> li = new ArrayList<>();
		boolean scan=false;
		int startPos = -1;
		...
		for(int i = 0;i < l.length();i++)
		{
			...
			if (isDigit(c))
			{
				...
				if (scan)
				{
					li.add(new NumbersInLineInfo(sb.toString(),startPos,i-1));
					scan = false;
					sb.setLength(0);
				}
			}
		...
		if (scan)
		{
			li.add(new NumbersInLineInfo(sb.toString(),startPos,l.length()-1));
		}
	...
	private static boolean checkSymbol(ArrayList<String> lines, int lnr, int start, int end) 
	{
		// Near the number must be a symbol, all other digig or . (point)
		
		// Check symbol in the line
		String l = lines.get(lnr);
		if (start > 0 && isSymbol(l.charAt(start -1))) return true;
		if (end < l.length() - 1 && isSymbol(l.charAt(end + 1))) return true;
		
		// Check Line above
		if (lnr > 0)
		{
			l = lines.get(lnr - 1);
			int startScannerPos = Math.max(start -1, 0);
			int endScannerPos = Math.min(end + 1,l.length() - 1);
			for(int i = startScannerPos;i <= endScannerPos;i++)
			{
				if (isSymbol(l.charAt(i))) return true;
			}
		}
		// check line below
		if (lnr < lines.size() - 1)
