...
public static String testInput0 = // 4 
"""
..........
.S------7.
.|F----7|.
...
public static String testInput1 = // 4 
"""
...........
.S-------7.
.|F-----7|.
...
public static String testInput2 = // 8 
"""
.F----7F7F7F7F-7....
.|F--7||||||||FJ....
.||.FJ||||||||L7....
...
public static String testInput3 = // 10 
"""
FF7FSF7F7F7F7F7F---7
L|LJ||||||||||||F--J
FL-7LJLJ||||||LJL-77
...
public static String testInput4 = // 1 
"""
.....
.S-7.
.|.|.
...
*/
	public static String input = testInput4;
//    public static String input = realInput;  // 6682
	
	public static int[] DIFFHELPER = {-1,1};
	public static void main(String[] args) throws Exception 
	{
		...
		ArrayList<String> pipeplan = new ArrayList<>();
...
//		printPipeplan(pipeplan);
		XYPoint startPoint = null;
		// Find startpoint
		for(int i = 0;i < pipeplan.size();i++)
		{
			int pos = pipeplan.get(i).indexOf('S');
			if (pos != -1)
			{
				if (startPoint != null)
				{
					System.out.println("Mehr als ein Startpoint");
					System.exit(222);
				}
				startPoint = new XYPoint(pos,i);
			}
		}
		
		System.out.println("Start=" + startPoint);
		ArrayList<XYPoint> twest = new ArrayList<>();
		ArrayList<XYPoint> teast = new ArrayList<>();
		ArrayList<XYPoint> tsouth = new ArrayList<>();
		ArrayList<XYPoint> tnorth = new ArrayList<>();

		// To north
		char c = getPoint(startPoint.x,startPoint.y - 1,pipeplan);
		
		if (c == '|' || c == 'F' || c == '7')
		{
			System.out.println("Start to north");
			tnorth = travel(startPoint,startPoint.x,startPoint.y - 1,EcurrentDirection.north, pipeplan);
			System.out.println(tnorth);
			checkInseln(tnorth,pipeplan);
		} 
				
		// To south
		c = getPoint(startPoint.x,startPoint.y + 1, pipeplan);
		
		
		if (c == '|' || c == 'L' || c == 'J')
		{
			System.out.println("Start to south");
			tsouth = travel(startPoint,startPoint.x,startPoint.y + 1,EcurrentDirection.south, pipeplan);
			System.out.println(tsouth);
			checkInseln(tsouth,pipeplan);
		} 

		// To west
		c = getPoint(startPoint.x-1,startPoint.y,pipeplan);
		
		if (c == '-' || c == 'F' || c == 'L')
		{
			System.out.println("Start to west");
			twest = travel(startPoint,startPoint.x - 1,startPoint.y,EcurrentDirection.west, pipeplan);
			System.out.println(twest);
			checkInseln(twest,pipeplan);
		} 
		...
		if (c == '-' || c == 'J' || c == '7')
		{
			System.out.println("Start to east");
			teast = travel(startPoint,startPoint.x + 1,startPoint.y,EcurrentDirection.east, pipeplan);
			System.out.println(teast);
			checkInseln(teast,pipeplan);
		}
		
		System.exit(0);
		ArrayList<XYPoint> check1 = tsouth;
		ArrayList<XYPoint> check2 = twest;
		
		for(XYPoint p : check1)
		{
			int i = check2.indexOf(p);
			int j = check1.indexOf(p);
			if (i != -1)
			{
				//System.out.println("In west = " + twest.indexOf(p)  + " in east " + i);
				if (i == j)
				{
					System.out.println("Gleich in " + i);
				}
			}
		}
...
. is ground; there is no pipe in this tile.
	...
	private static void checkInseln(ArrayList<XYPoint> way, ArrayList<String> pipeplan) 
	{
/*
.....
.S-7.
 ...
 */
		...
		int xmax = 0;
		ArrayList<XYPoint> insidePoints = new ArrayList<>();
		for(int y = 0;y < pipeplan.size();y++)
		{
			String row = pipeplan.get(y);
			xmax = row.length();
			boolean inside = false;
			boolean hasluecke = false;
...
*/		
	...
	private static ArrayList<XYPoint> travel(XYPoint startPoint, int xin, int yin, EcurrentDirection dir, ArrayList<String> pipeplan) 
	{
...
//			System.out.println(ps);
			...
			if (c == 'O' || c == '.') 
			{
				System.out.println("Fatal at " + x + ";" + y  + " as " + c);
				System.exit(333);
			}
			if (c == '|') {      
				if (dir == EcurrentDirection.north)
				{
					y--;
				} else {
					y++;
			...
			if (c == 'L') 
			{
				if (dir == EcurrentDirection.south)
				{
					x++;
					dir = EcurrentDirection.east;	
				} else if (dir == EcurrentDirection.west)
				{
					y--;
					dir = EcurrentDirection.north;	
				} else {
					System.out.println("Hier L");
					System.exit(10);
				}
			} else 
			...
			if (c == 'J') 
			{      
				if (dir == EcurrentDirection.east)
				{
					y--;
					dir = EcurrentDirection.north;	
				} else if (dir == EcurrentDirection.south)
				{
					x--;
					dir = EcurrentDirection.west;	
				} else {
					System.out.println("Hier J");
					System.exit(11);
				}
			} else
			...
			{      
				if (dir == EcurrentDirection.north)
				{
					x--;
					dir = EcurrentDirection.west;	
				} else if (dir == EcurrentDirection.east)
				{
					y++;
					dir = EcurrentDirection.south;	
				} else {
	...
	private static char getPoint(int x, int y, ArrayList<String> pipeplan) 
	{
		if (x < 0 || y < 0 || x > pipeplan.get(0).length() || y >= pipeplan.size()) return 'O'; // Outofbound
		
		return pipeplan.get(y).charAt(x);
	}


	private static void printPipeplan(ArrayList<String> pipeplan)
	{
		for(String s : pipeplan)
		{
