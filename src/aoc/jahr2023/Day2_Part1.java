...
""";
	...
	public static String[] colorTags = {"red","green","blue"};
	
	public static int[] frameConditionForMax = {12,13,14};
	...
	public static void main(String[] args) throws Exception 
	{
		...
		StringReader sr = new StringReader(realInput);
...
schleife:		
		while (true)
		{
			...
			for(String g : tok2)
			{
				String[] fs = g.trim().split(",");
				
				for(String f : fs)
				{
					String[] tok3 = f.trim().split(" ");
					for(int i = 0;i < colorTags.length;i++)
					{
						if (tok3[1].trim().equals(colorTags[i]))
						{
							int x = Integer.valueOf(tok3[0].trim());
							if (x > frameConditionForMax[i])
							{
								System.out.println("Violated for " + g + " in " + line);
								continue schleife; 
							}
						}
					}
			...
			int id = Integer.valueOf(tok1[0].replace("Game ",""));
			sumOfIds += id;
		}
