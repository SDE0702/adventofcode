...
//	public static String input = testInput1;
...
//			System.out.println(l);
			...
			StringBuilder receivedNumber = new StringBuilder();
			for(int j = 0;j < l.length();j++)
			{
				if (l.charAt(j) >= '0' && l.charAt(j) <= '9')
				{
					if (!scaned)
					{
						scaned = true;
					...
					receivedNumber.append(l.charAt(j));
				} else {
					if (scaned)
					{
						scaned = false;
						int rn = Integer.valueOf(receivedNumber.toString());
//						System.out.println(" - " + rn  + " for " + startpos + " to " + j);
						if (checkSymbol(lines,i,startpos,j-1))
						{
							sumOfOk += rn;
						}
						receivedNumber.setLength(0);
					}
				}
			}
			if (scaned)
			{
				int rn = Integer.valueOf(receivedNumber.toString());
//				System.out.println(" - " + rn  + " for " + startpos + " to " + (l.length()-1));
				if (checkSymbol(lines,i,startpos,l.length()-1))
				{
					sumOfOk += rn;
				}
			}
		}
		System.out.println(sumOfOk);
	}
